...
bool LoadTGA(textureObj * texture, char * filename)				// Load a TGA file
{
	...
	{
		cout << "Could not open texture file";	// Display an error message
		return false;														// Exit function
	}

	if(fread(&tgaheader, sizeof(TGAHeader), 1, fTGA) == 0)					// Attempt to read 12 byte header from file
	{
		cout << "Could not read file header";		// If it fails, display an error message 
	...
	{
		cout << "TGA file be type 2 or type 10 ";	// Display an error
		fclose(fTGA);
		return false;																// Exit function
	}
...
bool LoadUncompressedTGA(textureObj * texture, char * filename, FILE * fTGA)	// Load an uncompressed TGA (note, much of this code is based on NeHe's 
{																			// TGA Loading code nehe.gamedev.net)
	if(fread(tga.header, sizeof(tga.header), 1, fTGA) == 0)					// Read TGA header
	{										
		cout << "Could not read info header";		// Display error
	...
	{
		cout << "Invalid texture information";	// Display Error
	...
	{
		cout << "Could not allocate memory for image";	// Display Error
	...
	{
		cout << "Could not read image data";		// Display Error
...
bool LoadCompressedTGA(textureObj * texture, char * filename, FILE * fTGA)		// Load COMPRESSED TGAs
{ 
	...
	{
		cout << "Invalid texture information";	// If it isnt...Display error
	...
	do
	{
		...
		{
			cout << "Could not read RLE header";	// Display Error
			if(fTGA != NULL)												// If file is open
			{
				fclose(fTGA);												// Close file
			}
		...
		{																	// that follow the header
			...
			for(short counter = 0; counter < chunkheader; counter++)		// Read RAW color values
			{
				...
				{
					cout << "Could not read image data";		// IF we cant, display an error
				...
				{
					cout << "Too many pixels read";			// if there is too many... Display an error!
		...
		else																			// chunkheader > 128 RLE data, next color reapeated chunkheader - 127 times
		{
			...
			{	
				cout << "Could not read from file";			// If attempt fails.. Display error (again)
